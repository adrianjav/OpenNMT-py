...
string intercalate(const string &word){
   ...
   auto pos = word.begin();
   while(pos != word.end()){
      auto to = pos; // word.begin() + distance(word.begin(), pos);
      utf8::advance(to, 1, word.end());
      result += word.substr(distance(word.begin(), pos), distance(pos, to)) + special_symbols_utf8[" "];
      pos = to;
   }
   return result;
}
